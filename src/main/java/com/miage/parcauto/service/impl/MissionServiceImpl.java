...
public class MissionServiceImpl implements MissionService {
    ...
    private final PersonnelRepository personnelRepository; // Ou UtilisateurRepository si idPersonnel est dans
                                                           // Utilisateur
    ...
    public MissionServiceImpl() {
        ...
        this.vehiculeService = new VehiculeServiceImpl(); // Attention aux dépendances cycliques si VehiculeService
                                                          // utilise MissionService
    }
    ...
    @Override
    public MissionDTO createMission(MissionDTO missionDTO) throws ValidationException, VehiculeNotFoundException,
            UtilisateurNotFoundException, OperationFailedException {
        // validationService.validateMission(missionDTO);
        if (missionDTO == null || missionDTO.getIdVehicule() == null || missionDTO.getDateDebut() == null
                || missionDTO.getDateFinPrevue() == null) {
        ...
        // if (missionDTO.getIdPersonnel() == null) { // Si le personnel est obligatoire
        // pour une mission
        // throw new ValidationException("Un personnel doit être assigné à la
        // mission.");
        // }
        ...
        try {
            ...
            List<VehiculeDTO> vehiculesDispo = vehiculeService.getVehiculesDisponibles(missionDTO.getDateDebut(),
                    missionDTO.getDateFinPrevue());
            boolean estDisponible = vehiculesDispo.stream()
                    .anyMatch(v -> v.getIdVehicule().equals(missionDTO.getIdVehicule()));
            if (!estDisponible) {
                throw new OperationFailedException("Le véhicule ID " + missionDTO.getIdVehicule()
                        + " n'est pas disponible pour la période demandée.");
            }

            // if (missionDTO.getIdPersonnel() != null && personnelRepository.findById(conn,
            // missionDTO.getIdPersonnel()).isEmpty()) {
            // throw new UtilisateurNotFoundException("Personnel non trouvé avec l'ID: " +
            // missionDTO.getIdPersonnel());
            // }
    ...
    @Override
    public List<MissionDTO> getMissionsByVehiculeId(Integer idVehicule)
        ...
        try {
            ...
            throw new OperationFailedException(
                    "Erreur technique lors de la récupération des missions pour le véhicule ID: " + idVehicule, e);
        } finally {
            DbUtil.close(conn);
        }
    }
    ...
    @Override
    public List<MissionDTO> getMissionsByPersonnelId(Integer idPersonnel)
        ...
        try {
            ...
            // idPersonnel);
            // }
            // List<Mission> missions = missionRepository.findByPersonnelId(conn,
            // idPersonnel); // Méthode à ajouter au repo
            // return missionMapper.toDTOList(missions);
            return Collections.emptyList(); // Placeholder
        } catch (SQLException e) {
            throw new OperationFailedException(
                    "Erreur technique lors de la récupération des missions pour le personnel ID: " + idPersonnel, e);
        } finally {
            DbUtil.close(conn);
        }
    }
    ...
    @Override
    public MissionDTO updateMission(MissionDTO missionDTO) throws ValidationException, MissionNotFoundException,
        ...
        try {
            ...
            Mission existingMission = missionRepository.findById(conn, missionDTO.getIdMission())
                    .orElseThrow(() -> new MissionNotFoundException(
                            "Mission non trouvée avec l'ID: " + missionDTO.getIdMission()));

            // Logique de validation pour les modifications (ex: on ne modifie pas une
            // mission clôturée)
            if (existingMission.getStatut() == StatutMission.CLOTUREE && missionDTO.getStatut() != null
                    && !StatutMission.CLOTUREE.name().equals(missionDTO.getStatut())) {
                throw new ValidationException(
                        "Une mission clôturée ne peut pas changer de statut (sauf potentiellement pour réouverture admin).");
            }

            if (missionDTO.getIdVehicule() != null
                    && !missionDTO.getIdVehicule().equals(existingMission.getIdVehicule())) {
                if (vehiculeRepository.findById(conn, missionDTO.getIdVehicule()).isEmpty()) {
                    throw new VehiculeNotFoundException(
                            "Nouveau véhicule associé non trouvé avec l'ID: " + missionDTO.getIdVehicule());
                }
                // Vérifier disponibilité si dates changent ou véhicule change
                LocalDateTime debutCheck = missionDTO.getDateDebut() != null ? missionDTO.getDateDebut()
                        : existingMission.getDateDebutMission();
                LocalDateTime finCheck = missionDTO.getDateFinPrevue() != null ? missionDTO.getDateFinPrevue()
                        : existingMission.getDateFinMission();
                if (debutCheck != null && finCheck != null) {
                    ...
                    boolean estDisponible = vehiculesDispo.stream()
                            .anyMatch(v -> v.getIdVehicule().equals(missionDTO.getIdVehicule()));
                    if (!estDisponible && !missionRepository
                            .findOverlappingMissionsForVehicule(conn, missionDTO.getIdVehicule(), debutCheck, finCheck)
                            .stream().allMatch(m -> m.getIdMission().equals(existingMission.getIdMission()))) { // Exclure
                                                                                                                // la
                                                                                                                // mission
                                                                                                                // actuelle
                                                                                                                // de la
                                                                                                                ...
                                                                                                                // conflit
                        throw new OperationFailedException("Le véhicule ID " + missionDTO.getIdVehicule()
                                + " n'est pas disponible pour la période modifiée.");
                    }
                }
                existingMission.setIdVehicule(missionDTO.getIdVehicule());
            }

            // if (missionDTO.getIdPersonnel() != null &&
            // !missionDTO.getIdPersonnel().equals(existingMission.getIdPersonnel())) {
            // if (personnelRepository.findById(conn,
            // missionDTO.getIdPersonnel()).isEmpty()) {
            // throw new UtilisateurNotFoundException("Nouveau personnel associé non trouvé
            // avec l'ID: " + missionDTO.getIdPersonnel());
            // }
            // existingMission.setIdPersonnel(missionDTO.getIdPersonnel());
            // }

            if (missionDTO.getLibelle() != null)
                existingMission.setLibelle(missionDTO.getLibelle());
            if (missionDTO.getSiteDestination() != null)
                existingMission.setSite(missionDTO.getSiteDestination());
            if (missionDTO.getDateDebut() != null)
                existingMission.setDateDebutMission(missionDTO.getDateDebut());
            if (missionDTO.getDateFinPrevue() != null)
                existingMission.setDateFinMission(missionDTO.getDateFinPrevue());
            if (missionDTO.getDateFinEffective() != null)
                existingMission.setDateFinEffective(missionDTO.getDateFinEffective());
            if (missionDTO.getKmPrevu() != null)
                existingMission.setKmPrevu(missionDTO.getKmPrevu());
            if (missionDTO.getKmReel() != null)
                existingMission.setKmReel(missionDTO.getKmReel());
            if (missionDTO.getStatut() != null)
                existingMission.setStatut(StatutMission.valueOf(missionDTO.getStatut()));
            if (missionDTO.getCoutEstime() != null)
                existingMission.setCoutEstime(missionDTO.getCoutEstime());
            if (missionDTO.getCoutTotalReel() != null)
                existingMission.setCoutTotal(missionDTO.getCoutTotalReel());
            if (missionDTO.getCircuit() != null)
                existingMission.setCircuitMission(missionDTO.getCircuit());
            if (missionDTO.getObservations() != null)
                existingMission.setObservationMission(missionDTO.getObservations());
    ...
    @Override
    public MissionDTO terminerMission(Integer idMission, int kmRetour, LocalDateTime dateFinEffective,
            String observations) throws MissionNotFoundException, ValidationException, OperationFailedException {
        if (kmRetour < 0) {
        ...
        try {
            ...
            if (mission.getDateDebutMission() != null && dateFinEffective.isBefore(mission.getDateDebutMission())) {
                throw new ValidationException(
                        "La date de fin effective ne peut pas être antérieure à la date de début.");
            }

            Vehicule vehicule = vehiculeRepository.findById(conn, mission.getIdVehicule())
                    .orElseThrow(() -> new OperationFailedException(
                            "Véhicule associé à la mission non trouvé. ID: " + mission.getIdVehicule()));

            if (kmRetour < vehicule.getKmActuels()
                    && (mission.getKmPrevu() != null && kmRetour < mission.getKmPrevu())) { // Tolérance si kmPrevu
                                                                                            // était déjà plus bas
                System.err.println("Avertissement: Kilométrage de retour (" + kmRetour
                        + ") inférieur au kilométrage actuel du véhicule (" + vehicule.getKmActuels()
                        + ") ou au km prévu (" + mission.getKmPrevu() + ").");
            }

            mission.setKmReel(kmRetour);
            mission.setDateFinEffective(dateFinEffective);
            mission.setStatut(StatutMission.CLOTUREE);
            mission.setObservationMission(
                    (mission.getObservationMission() == null ? "" : mission.getObservationMission() + "\n")
                            + "Clôture: " + (observations == null ? "" : observations));

            // Le trigger `trg_mission_cloturee` dans la DB devrait mettre à jour
            // `VEHICULES.km_actuels`
    ...
    @Override
    public DepenseMissionDTO ajouterDepenseAMission(Integer idMission, DepenseMissionDTO depenseDTO)
    ...
    @Override
    public List<DepenseMissionDTO> getDepensesByMissionId(Integer idMission)
        ...
        try {
            ...
            throw new OperationFailedException(
                    "Erreur technique lors de la récupération des dépenses pour la mission ID: " + idMission, e);
        } finally {
            DbUtil.close(conn);
        }
    }
    ...
    @Override
    public List<MissionDTO> getMissionsActivesPourVehicule(Integer idVehicule)
            throws VehiculeNotFoundException, OperationFailedException {
        Connection conn = null;
        try {
            conn = DbUtil.getConnection();
    ...
    @Override
    public List<MissionDTO> getMissionsParPeriode(LocalDateTime dateDebut, LocalDateTime dateFin)
